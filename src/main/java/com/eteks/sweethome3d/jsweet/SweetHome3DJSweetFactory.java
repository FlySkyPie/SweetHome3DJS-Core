...
public class SweetHome3DJSweetFactory extends JSweetFactory {
   ...
   * @author Renaud Pawlak
   */
  public static class SAXParserJSweetAdapter extends PrinterAdapter {
    public SAXParserJSweetAdapter(PrinterAdapter parent) {
      ...
      // Fix jsXmlSaxParser which doesn't replace quot lt gt amp entities in parsed
      // text
      addAnnotation(
        ...
        switch (targetType.toString()) {
          case "org.xml.sax.Attributes":
            if ("getValue".equals(invocation.getMethodName())) {
              // Fix jsXmlSaxParser which doesn't replace quot lt gt amp entities in parsed
              // text
              print(invocation.getTargetExpression()).print(".getValue(").print(invocation.getArguments().get(0)).print(
   ...
   * It's used temporarily to help faster transpiling Java 3D API calls to
   * glMatrix and scene3d.js ones.
   * 
   * @author Emmanuel Puybaret
   * @author Renaud Pawlak
   */
  public static class SweetHome3DJava3DJSweetAdapter extends PrinterAdapter {
      ...
      if (invocation.getTargetExpression() != null) {
        switch (invocation.getTargetExpression().getTypeAsElement().toString()) {
          case "javax.media.j3d.Appearance":
            switch (invocation.getMethodName()) {
              ...
              case "setRenderingAttributes":
                ...
                return true;
              case "setTexture":
                print(invocation.getTargetExpression()).print(".setTextureImage").print("(")
          ...
          case "com.sun.j3d.utils.geometry.NormalGenerator":
            switch (invocation.getMethodName()) {
              case "setCreaseAngle":
                print(this.lastGeometryInfoName).print(".").print(invocation.getMethodName()).print("(")
          ...
          case "javax.media.j3d.Transform3D":
            switch (invocation.getMethodName()) {
              case "setTranslation":
                print("mat4.fromTranslation(").print(invocation.getTargetExpression()).print(", ")
                    .printArgList(invocation.getArguments()).print(")");
                return true;
              case "rotX":
                print("mat4.fromXRotation(").print(invocation.getTargetExpression()).print(", ")
                    .printArgList(invocation.getArguments()).print(")");
                return true;
              case "rotY":
                print("mat4.fromYRotation(").print(invocation.getTargetExpression()).print(", ")
                    .printArgList(invocation.getArguments()).print(")");
                return true;
              case "rotZ":
                print("mat4.fromZRotation(").print(invocation.getTargetExpression()).print(", ")
                    .printArgList(invocation.getArguments()).print(")");
                return true;
              case "setScale":
                print("mat4.scale(").print(invocation.getTargetExpression()).print(", ")
                    .print(invocation.getTargetExpression()).print(", ").printArgList(invocation.getArguments())
                    .print(")");
                return true;
              case "mul":
                print("mat4.mul(").print(invocation.getTargetExpression()).print(", ")
                    .print(invocation.getTargetExpression()).print(", ").printArgList(invocation.getArguments())
                    .print(")");
                return true;
              case "transform":
                print("vec3.transformMat4(").print(invocation.getArguments().get(0)).print(", ")
                    .print(invocation.getArguments().get(0)).print(", ").print(invocation.getTargetExpression())
                    .print(")");
                return true;
              case "invert":
                print("mat4.invert(").print(invocation.getTargetExpression()).print(", ")
                    .printArgList(invocation.getArguments()).print(")");
                return true;
            }
            break;
   ...
   * @author Renaud Pawlak
   */
  public static class SweetHome3DJSweetAdapter extends PrinterAdapter {
    ...
    public SweetHome3DJSweetAdapter(PrinterAdapter parent) {
      ...
      addAnnotation("jsweet.lang.Erased",
          ...
          // Remove overloaded addPropertyChangeListener / removePropertyChangeListener
          // methods to manage function listeners case
          ...
          // Remove overloaded addPropertyChangeListener / removePropertyChangeListener
          // methods because only their form with a string propertyName is used
        ...
        public Action manageAnnotation(Element element, String annotationType) {
          ...
          if (JSweetConfig.ANNOTATION_ERASED.equals(annotationType)) {
            if (element.getKind() == ElementKind.ENUM && element.getSimpleName().toString().endsWith("Property")) {
              // All enums named *Property will be erased (because they will be strings in the
              // generated code)
              return Action.ADD;
              ...
              // (except the one with 18 parameters where backFaceShown boolean 12th parameter
              // was replaced by modelFlags int parameter)
              ...
              if (!element.getModifiers().contains(Modifier.PRIVATE)) {
                if (c.getParameters().size() != 16
                    ...
                    && (c.getParameters().size() != 18
                        || types().isSameType(c.getParameters().get(11).asType(), util().getType(boolean.class)))) {
                  return Action.ADD;
                }
              }
              ...
              // Only keep the public constructor of HomeController3D which doesn't use
              // PlanController parameter
              ExecutableElement c = (ExecutableElement) element;
      ...
      if (invocation.getTargetExpression() != null) {
        ...
        switch (targetType.toString()) {
          ...
          case "java.util.Arrays":
            switch (invocation.getMethodName()) {
              // WARNING: we assume that this method will be used to log arrays so we just
              // pass the array as is
            ...
            switch (invocation.getMethodName()) {
              ...
              case "equals":
                ...
                print("((").print(invocation.getArguments().get(0)).print(") != null ? ")
                    ...
                    .print(") : (")
                    .print(invocation.getTargetExpression()).print(" === (").print(invocation.getArguments().get(0))
                    .print(")))");
                return true;
            }
            break;
          case "java.lang.String":
            switch (invocation.getMethodName()) {
              case "split":
                ...
                String regex = invocation.getArgument(0).toString().replaceAll("\\\\s", "\\s").replaceAll("\\\\p",
                    "\\p");
                print(invocation.getTargetExpression()).print(".").print(invocation.getMethodName()).print("(/")
              ...
              case "getDefaultFractionDigits":
                if (invocation.getTargetExpression() instanceof MethodInvocationElement) {
                  if ("getInstance".equals(
                      ((MethodInvocationElement) invocation.getTargetExpression()).getMethod().getSimpleName()
                          .toString())) {
                    printMacroName(invocation.getMethodName());
      ...
      if (element instanceof TypeElement) {
        ...
        if (types().isSubtype(type.asType(), util().getType(Throwable.class))
            ...
            || type.getQualifiedName()
                .contentEquals("com.eteks.sweethome3d.viewcontroller.ImportedFurnitureWizardController")) {
          newComment.append("\n");
